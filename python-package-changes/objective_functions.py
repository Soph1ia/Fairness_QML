...
class FairnessObjectiveFunction(ObjectiveFunction, testData):
    # hardcoded for now
    ...
    def objective(self, weights: np.ndarray) -> float:
        #
        print("Neural network is ",self._neural_network)
        print("Fairness objective function")
        print("the weights are: ", weights)
        print("self._X: ", self._X)

        # get the prediction for the current neural network
        predict = self._neural_network_forward(weights)
        # convert the prediction to classification labels
        ...
        # construct the StandardDataset
        train_dataset = StandardDataset(data, label_name="target", favorable_classes=[1], protected_attribute_names=['gender'], privileged_classes=[[1]])

        self.testData = testData

        # hardcoded value, we need the test dataset
        test_dataset = StandardDataset(self.testData,
                                       label_name="target",
                                       favorable_classes=[1],
    ...
    def gradient(self, weights: np.ndarray) -> np.ndarray:
        print("Fairness gradient function")
        print("the weights are: ", weights)
        return -100

    # convert data into standard dataset , we have self.y which is the actual
    def convert_to_standard_dataset(self):
        ...
        # convert the y into standard dataset

        return dataset


    # convert the predicted class into standard dataset

    # put both test and predicted dataset into classification metric

    # test the fairness for different metrics.